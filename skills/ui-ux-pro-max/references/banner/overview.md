...
1. **Create HTML/CSS banner** using project frontend conventions
   - Use exact platform dimensions from size reference
   - Apply safe zone rules (critical content in central 70-80%)
   - Max 2 typefaces, single CTA, 4.5:1 contrast ratio
   - Inject brand context via `inject-brand-context.cjs`

2. **Generate visual elements** with available image-generation tooling

   **a) Search prompt inspiration** from any available prompt/example library:
   ```bash
   python3 image generation prompt search tooling "<banner style keywords>"
   ```

   **b) Generate with Standard model** (fast, good for backgrounds/patterns):
   ```bash
   python path/to/image-generation-script.py \
...
- Exported PNG preview (use image-generation tooling skill to display if needed)

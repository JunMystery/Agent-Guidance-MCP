...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-pro-max`, `brand`, `design-system`, and browser automation skills.
...
1. **`references/brand/overview.md`** — Extract brand colors, fonts, voice from user's project
2. **`references/design-system/overview.md`** — Get design tokens (spacing, typography scale, color palette)
3. **Randomly invoke ONE of:** `skills/ui-ux-pro-max/SKILL.md` OR the project frontend conventions — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Playwright, Puppeteer, or another browser automation tool to capture exact-size screenshots.
...
#### Option B: browser automation skill

Invoke `/browser automation` with instructions to:
...
Use browser automation to visually inspect each exported PNG:
...
- AI image generation (use image-generation tooling skill for that)
